...
/*   Updated: 2023/07/29 17:40:31 by aputiev          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/minishell.h"

t_env_list	*deletenode(t_env_list *curr, char *name)	//ADD FREE NAME , FREE VAL
{
	t_env_list	*next;
...
t_env_list	*sort_list(t_env_list *lst)		//<-changed_23.07.28
{
	t_env_list	*tmp;
	char		*swap_name;
	char		*swap_value;
...
void	insert_at_end(t_env_list *head, char *name, char *value, int equal_sign)	//<-changed_23.07.28
{
	t_env_list	*node;
	t_env_list	*current;
	...
	node->equal_sign = equal_sign;	//<-changed_23.07.28
	node->next = NULL;
	current = head;
