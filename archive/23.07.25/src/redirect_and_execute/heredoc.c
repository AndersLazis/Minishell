...
/*   Updated: 2023/07/25 13:08:10 by aputiev          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	create_heredoc(t_data *data)
{
	...
	if (tmp_file)
		close(tmp_file);
	tmp_file = open("tmp/tmp_file", O_CREAT | O_RDWR | O_TRUNC, 0777);	//<-changed_23.07.25
	if (tmp_file== ERROR)
	{
...
char *implement_heredoc(char	*str)	
{
	...
	int	i;
	int	j;
	char	*tmpfile = "tmp/tmp_file";	//<-changed_23.07.25
...
{	
	...
	if (data->heredoc_flag != 1)
		return(NULL);
	if(create_heredoc(data) == EXIT_FAILURE)
		exit (0);
	tmp_str = implement_heredoc(str);
	if (tmp_str == NULL)
		exit (0);
	return(tmp_str);
}
